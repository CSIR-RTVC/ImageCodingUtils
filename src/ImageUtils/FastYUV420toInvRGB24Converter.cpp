...
FILE NAME					: FastYUV420toInvRGB24Converter.cpp
...
#include "FastYUV420toInvRGB24Converter.h"
...
===========================================================================
*/
#define FYUVIRGB24C_RANGECHECK_0TO255(x) ( (((x) <= 255)&&((x) >= 0))?((x)):( ((x) > 255)?(255):(0) ) )
...
===========================================================================
*/
void FastYUV420toInvRGB24Converter::NonRotateConvert(VICS_PSBYTE pY,VICS_PSBYTE pU,VICS_PSBYTE pV,VICS_POINTER pRgb)
{
	...
	for(y = 0,lumposy = 0,uvposy = 0,rgbposy = ((_height-1)*rgb1row);	y < lumY;	y += 2,lumposy += tworows,uvposy += uvX,rgbposy -= rgb2rows)
	{
		...
		for(x = 0; x < lumX; x += 2)
		{
  		...
  		// Fast calculation with 6 bit to 8 bit conversion built in. 
			...
			optr[rgbpos0++] = (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(b));
			optr[rgbpos0++] = (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(g));
			optr[rgbpos0++] = (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(r));
			...
			// R, G & B have range 0..255.
			optr[rgbpos1++] = (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(b));
			optr[rgbpos1++] = (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(g));
			optr[rgbpos1++] = (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(r));
...
void FastYUV420toInvRGB24Converter::RotateConvert(VICS_PSBYTE pY,VICS_PSBYTE pU,VICS_PSBYTE pV,VICS_POINTER pRgb)
{
	VICS_BYTE* 	optr	=	(VICS_BYTE *)pRgb;
	VICS_INT		lumX	= _width;
	VICS_INT		lumY	= _height;
...
//			VICS_INT ca =  (v >> 2) + (v >> 5) - 144;
  		...
  		// Fast calculation with 6 bit to 8 bit conversion built in. 
			...
			// R, G & B have range 0..255.
			optr[rgbpos0]		= (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(b));
			optr[rgbpos0+1] = (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(g));
			optr[rgbpos0+2] = (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(r));
			rgbpos0 += rgb1row;
			...
			optr[rgbpos1]		= (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(b));
			optr[rgbpos1+1] = (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(g));
			optr[rgbpos1+2] = (VICS_BYTE)(FYUVIRGB24C_RANGECHECK_0TO255(r));
			rgbpos1 += rgb1row;
