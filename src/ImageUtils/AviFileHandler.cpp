...
--------------------------------------------------------------------------
*/
// Limit the calls to AVIFileInit() and AVIFileExit() to 1 only.
int	AviFileHandler::_initialisations = 0;
...
int AviFileHandler::OpenRead(CString& filename)
{
  ...
  if(!_frame->CreateImage(&streamBitmapHead))
	{
    LPTSTR lpsz = new TCHAR[_frame->_errorStr.GetLength()+1];
    strcpy(lpsz, _frame->_errorStr);
  	_errorStr = lpsz;
    Close(); 
    return(0); 
	}//end if !CreateImage...
  ...
  _frameRate = (unsigned long)(((double)_streamInfo.dwRate)/((double)_streamInfo.dwScale)+ 0.5);
...
ImageHandler* AviFileHandler::GetNextImage(void)
{
  ...
  if( AVIStreamRead(_pVideoStream, _streamCount, 1,
  	                _frame->_bmptr, _frame->_bmih->biSizeImage,
  									NULL,NULL)
      != 0 )
    {
