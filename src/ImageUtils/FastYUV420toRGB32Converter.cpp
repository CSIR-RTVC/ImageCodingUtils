...
DESCRIPTION				: Fast YUV420 to RGB 32 bit colour convertion derived from
										the FastYUV420toRGBConverter base class. The bits are 
										packed in a 32 bit word.
	...
	for(y = 0,lumposy = 0,uvposy = 0,rgbposy = 0;	y < lumY;	y += 2,lumposy += tworows,uvposy += uvX,rgbposy += rgb2rows)
	{
		...
		for(x = 0; x < lumX; x += 2)
		{
  		...
  		// Fast calculation with 6 bit to 8 bit conversion built in. 
			...
			b = FYUVRGB32C_RANGECHECK_0TO255(b);
			g = FYUVRGB32C_RANGECHECK_0TO255(g);
			r = FYUVRGB32C_RANGECHECK_0TO255(r);
